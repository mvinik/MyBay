...
const FAQAccordion = () => {
  ...
  return (
    <div className={`${styles.paddingHorizontal} w-full py-16 flex flex-col gap-y-10 md:flex-row gap-12`}>
      ...
      <div className="w-full flex mt-8 flex-col lg:flex-row items-start justify-between lg:items-end gap-y-6 lg:gap-y-0">
        ...
        {faqs.map((faq, index) => (
          <li key={index}>
            ...
            >
              ...
              <svg
                className={`flex-shrink-0 w-4 h-4 ml-auto fill-current transition-transform duration-300 ${openIndex === index ? "rotate-45" : ""
                  }`}
                viewBox="0 0 16 16"
            ...
            >
              <div className="pb-5 leading-relaxed">
                <div className="space-y-2 text-sm text-gray-600 leading-relaxed">{faq.answer}</div>
              </div>
            </div>
          </li>
        ))}
